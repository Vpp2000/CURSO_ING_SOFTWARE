...
to go
  ...
  ask turtles with [self != selected-car and ((xcor > xcor-of-selected-turtle and ycor = ycor-of-selected-turtle and ([distance selected-car] of self) < 2.9) or patience <= 0)] [ choose-new-lane ]
  ask selected-car [if patience <= 0 [choose-new-lane]]
...
to select-car
  ...
  if mouse-down? [
    ...
    if any? turtles-on patch mx my [
      ...
      ask selected-car [ set color red ]
      ask selected-car [ set speed top-speed ]
...
to-report ycor-of-selected-turtle
  report [ycor] of selected-car
end

to-report xcor-of-selected-turtle
  report [xcor] of selected-car
end
