...
globals [
  ...
  lanes          ; a list of the y coordinates of different lanes in which you can draw a car
  number-of-patches ; with faith
  selected-truck
  lanes-truck
]
...
cars-own [
  ...
  patience      ; the drivers current level of patience
]
...
to setup
  ...
  set lanes-truck remove-first-n  lanes 1
  set lanes sublist lanes 1 (length lanes)

  create-or-remove-cars
  create-or-remove-trucks

  set selected-car one-of cars
  ask selected-car [ set color red ]

  set selected-truck one-of trucks
  ask selected-truck [ set color red ]
...
to create-or-remove-cars
  ...
  ; make sure we dont have too many cars for the room we have on the road
  ...
  create-cars (number-of-cars - count cars) [
    ...
    set heading 90  ;; NO SE QUE HACE ESTA VARIABLE
...
to create-or-remove-trucks

  ; make sure we don't have too many cars for the room we have on the road
  let road-patches patches with [ member? pycor lanes-truck ]
  if number-of-trucks > count road-patches [
    set number-of-trucks count road-patches
  ]
  ...
  if count trucks > number-of-trucks [
    let n count trucks - number-of-trucks
    ask n-of n [ other trucks ] of selected-truck [ die ]
  ]

end
;;****************** end of created function ********************

to-report free [ road-patches ] ; car procedure
  let this-car self
...
;;****************** adding the next function ********************
to-report free-truck [ road-patches ] ; truck procedure
  let this-truck self
  report road-patches with [
    not any? trucks-here with [ self != this-truck ]
  ]
end
;;****************** end of created function ********************
...
to draw-line [ y line-color gap ]
  ...
  create-cars 1 [
    ...
    set heading 90   ;; ANGULO DE LAS LINEAS DEL CARRO
    repeat world-width [
      pen-up
      forward gap
      pen-down
...
to go
  ...
  create-or-remove-trucks
  ask trucks [ move-forward-truck ]
  ask trucks with [ patience <= 0 ] [ choose-new-lane-truck ]
  ask trucks with [ ycor != target-lane ] [ move-to-target-lane-truck ]
...
to move-forward ; car procedure
  set heading 90  ;; ESTA VARIABLE INDICA EN QUE SENTIDO SE MUEVEN LOS AUTITOS
...
;;****************** adding the next function ********************
to move-forward-truck ; truck procedure
  set heading 90
  speed-up-truck ; we tentatively speed up, but might have to slow down
  let blocking-trucks other trucks in-cone (1 + speed) 180 with [ y-distance <= 1 ]
  let blocking-truck min-one-of blocking-trucks [ distance myself ]
  if blocking-truck != nobody [
    ; match the speed of the car ahead of you and then slow
    ; down so you are driving a bit slower than that car.
    set speed [ speed ] of blocking-truck
    slow-down-truck
  ]
  forward speed
end

to slow-down-truck ; truck procedure
  set speed (speed - deceleration)
  if speed < 0 [ set speed deceleration ]
  ; every time you hit the brakes, you loose a little patience
  set patience patience - 1
...
to speed-up-truck ; truck procedure
  set speed (speed + acceleration)
  if speed > top-speed [ set speed top-speed ]
end
;;****************** end of created function ********************
...
to choose-new-lane-truck ; truck procedure
  ; Choose a new lane among those with the minimum
  ; distance to your current lane (i.e., your ycor).
  let other-lanes remove ycor lanes-truck
  if not empty? other-lanes [
    let min-dist min map [ y -> abs (y - ycor) ] other-lanes
    let closest-lanes filter [ y -> abs (y - ycor) = min-dist ] other-lanes
    set target-lane one-of closest-lanes
...
to move-to-target-lane-truck ; truck procedure
  set heading ifelse-value target-lane < ycor [ 180 ] [ 0 ]
  let blocking-trucks other trucks in-cone (1 + abs (ycor - target-lane)) 180 with [ x-distance <= 1 ]
  let blocking-truck min-one-of blocking-trucks [ distance myself ]
  ifelse blocking-truck = nobody [
    forward 0.2
    set ycor precision ycor 1 ; to avoid floating point errors
  ] [
    ; slow down if the car blocking us is behind, otherwise speed up
    ifelse towards blocking-truck <= 180 [ slow-down-truck ] [ speed-up-truck ]
  ]
end
...
to select-truck
  ...
  if mouse-down? [
    let mx mouse-xcor
    let my mouse-ycor
    if any? trucks-on patch mx my [
      ask selected-truck [ set color truck-color ]
      set selected-truck one-of trucks-on patch mx my
      ask selected-truck [ set color red ]
      display
    ]
  ]
...
to-report remove-first-n [ lst n ]
